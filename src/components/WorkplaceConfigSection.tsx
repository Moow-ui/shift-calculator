...
export const WorkplaceConfigSection: React.FC<WorkplaceConfigSectionProps> = ({
  ...
  return (
    <section id="config-section" className="space-y-4 w-full min-w-0">
      ...
      {/* ────────────────────────────────────────────────────────── */}
      <div className="bg-white rounded-2xl border border-slate-300/90 shadow-2xs p-3.5 sm:p-4 space-y-2.5">
        <div className="flex items-center justify-between pb-2 border-b border-slate-100">
          <div className="flex items-center gap-2 min-w-0">
            <div className="w-6 h-6 rounded-lg bg-indigo-100 text-indigo-700 flex items-center justify-center font-black text-xs flex-shrink-0">
              1
            </div>
            <label className="text-xs sm:text-sm font-extrabold text-slate-900 flex items-center gap-1.5 truncate">
              <Users className="w-4 h-4 text-indigo-600 flex-shrink-0" />
              <span>알바하는 곳에 직원이 5명이 넘나요?</span>
            </label>
          </div>
          <span className="text-[10px] sm:text-[11px] font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded-full border border-indigo-200 flex-shrink-0">
            상시 근로자 수
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
          ...
          >
            ...
            <p className="text-[11px] text-slate-600 mt-1 leading-relaxed">
              밤 10시 이후 야간근무나 하루 8시간 넘게 일하면 <strong className="text-indigo-700 font-bold">50% 가산수당(1.5배)</strong>이 발생합니다.
            </p>
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-300/90 shadow-2xs p-3.5 sm:p-4 space-y-2.5">
        <div className="flex items-center justify-between pb-2 border-b border-slate-100">
          ...
          <span className="text-[10px] sm:text-[11px] font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200 flex-shrink-0">
            세금/공제 설정
          </span>
        </div>
        ...
        <div className="pt-1">
          <select
            ...
            className="w-full px-3 py-2.5 text-xs sm:text-sm bg-slate-50 border border-slate-300 rounded-xl font-bold text-slate-900 focus:ring-2 focus:ring-emerald-500/20 shadow-2xs cursor-pointer"
          >
          ...
          {config.probation.applied && (
            <div className="mt-3 p-3 bg-amber-50/70 rounded-xl border border-amber-300 space-y-2.5 shadow-2xs animate-fadeIn">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                <label className="flex items-start gap-2 cursor-pointer text-slate-800 bg-white p-2.5 rounded-lg border border-amber-200 shadow-2xs">
                  <input
                    type="checkbox"
                  ...
                  />
                  <div>
                    <span className="font-extrabold text-slate-900">1년 넘게 일하기로 계약했나요?</span>
                    <p className="text-[11px] text-slate-500 mt-0.5">(1년 미만 단기 알바는 시급 감액 절대 불가)</p>
                  </div>
                </label>
                ...
                <label className="flex items-start gap-2 cursor-pointer text-slate-800 bg-white p-2.5 rounded-lg border border-amber-200 shadow-2xs">
                  ...
                  />
                  <div>
                    <span className="font-extrabold text-slate-900">편의점, 카페, 식당 서빙, 배달 같은 일인가요?</span>
                    <p className="text-[11px] text-slate-500 mt-0.5">(단순노무직종은 수습이라도 감액 절대 불가)</p>
                  </div>
                </label>
              </div>
              ...
              {probationBanner && (
                ...
                >
                  ...
                  <div>
                    <div className="font-extrabold">{probationBanner.title}</div>
                    <p className="text-[11px] leading-relaxed opacity-90 mt-0.5">{probationBanner.text}</p>
                  </div>
                </div>
              )}
            </div>
