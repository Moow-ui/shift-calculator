...
## 이 저장소
- 역할: 알바 급여 계산기(shift-calculator). 알바생이 근무를 입력하면 월급·주휴수당을 계산하는 React(Vite) 앱.
- 배포: main에 push하면 Cloudflare가 빌드 결과 폴더(dist/)만 배포한다. 없는 주소는 메인 화면으로 연결된다.
- 수정 금지 파일: src/lib/payroll.ts(급여 계산), src/config/holidays.ts·src/config/minimumWage.ts(공휴일·최저임금 값), src/types/payroll.ts
- 테스트 명령: npm test (기존 39개). 빌드 확인은 npm run build
